...
private:
    ...
    //Data member to store webpages that match a particular search term
    std::map<std::string, WebPageSet> terms_w_webpagaes; 

    //Data member to store webpage objects asscoiated with each file/page name
    std::map<std::string, WebPage* > fileNames_of_webpages;
    WebPage* add_new_webpage(const std::string& page_name );
};
